...
Arachne отделяет владение процессом от исполнения отдельного шага. Ядро знает,
какой шаг следующий и что ему передать. Паук знает, как разговаривать с одной
внешней системой. Ни один из них не обязан изображать бога монорепозитория.
...
NATS сериализует payload в JSON. Поэтому `Artifact`, `RunHandle`, `StepSpec` и
ошибки проходят явный wire codec, а не надеются на телепатию Python-процессов.
...
Каждый run сохраняет `scenario_version_id` и `scenario_snapshot`. Это защищает
историю от последующего редактирования опубликованного сценария.

...
Паук отвечает за один шаг и один внешний исполнитель. Шиной и сценарием управляет
ядро; thread adapter выставляет responder и упаковывает ошибки.
...
Запускайте сценарий через `self.fire`: общая точка входа создаст `Run`, сохранит
снимок сценария и подготовит live-state.
...
Для нового типа параметра придётся изменить:

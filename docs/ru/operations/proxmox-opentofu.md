...
## Длительные операции и отмена

Шаги `tofu-proxmox` не имеют общего дедлайна Арахны (остальные шаги
сохраняют лимит 2 часа). Это относится к созданию и удалению стендов,
включая `tofu init`, `apply`, `destroy` и чтение outputs.

У bpg/proxmox 0.111.1 нет бесконечного ожидания: нулевой таймаут операции
истекает сразу. В штатном модуле ожидания операций VM и guest agent заданы
как 2147483647 секунд (около 68 лет), то есть практически без ограничения.
Это конечный срок, а не специальное значение «бесконечность».
Внешние модули из `TOFU_ROOT` должны задавать собственные параметры ожидания.
Сетевые ошибки и ошибки Proxmox по-прежнему завершают шаг ошибкой.

Ручная отмена прерывает группу процессов OpenTofu через SIGINT; через
10 секунд незавершившаяся группа получает SIGKILL. Этот срок ограничивает
только очистку при отмене. Уже отправленная серверная задача Proxmox может
продолжить работу: отмена не откатывает созданную VM.

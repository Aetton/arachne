...
Краткая карта текущей реализации. Подробная версия:
[`docs/ru/architecture/overview.md`](docs/ru/architecture/overview.md).

## Главный принцип

Arachne управляет сценарием. Ядро выбирает шаг, подставляет входы и переносит
артефакты. Паук выполняет один шаг во внешней системе.
...
Паук работает в рамках одного шага. Сценарий, DAG, ACL и шина остаются на стороне
ядра, а адаптер выставляет паука на subjects:
...
`NatsBus` использует тот же контракт и JSON wire codec. С ним нити можно вынести
в другие процессы. Готового отдельного worker CLI в проекте пока нет.

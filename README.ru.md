...
артефакты. Сама тяжёлая работа остаётся там, где ей и положено быть: в Forgejo
Actions, Ansible, OpenTofu или другом подключённом исполнителе.
...
До запуска не на локальной машине обязательно замените `JWT_SECRET`,

...
Этот файл описывает seed-формат рядом с примером. Канонический полный справочник:
[`docs/ru/reference/scenario-syntax.md`](../docs/ru/reference/scenario-syntax.md).
Эталон seed-файла: [`scenarios.yaml.example`](scenarios.yaml.example).
...
| `type` | да | `string`, `choice`, `select` или `boolean` |
...
- `select` отображается как список; `source.type: git_branches` добавляет поиск веток Forgejo;
- `boolean` отображается как checkbox и передаётся настоящим boolean;
- остальные значения формы передаются строками;
- `default` задаёт начальное значение формы; автоматические запуски его не подставляют.

Для `schedule` перечислите нужные `params` явно. `chain` запускает следующий
сценарий с пустым словарём параметров.
...
Подстановки рекурсивно обрабатываются во вложенных словарях и списках любой глубины.
...
Запускает Forgejo Actions workflow, а затем сам читает статус, логи и артефакты
через Forgejo v16 API.
...
Служебные `build_id`, `arachne_callback` и `arachne_token` относились к старому
hub-контракту и оставлены для совместимости. Текущий паук их не добавляет. В workflow
объявляйте рабочие inputs, секреты храните в Forgejo secrets.

Основной артефакт обычно имеет тип `nexus`. Паук распознаёт URL вида
`.../repository/<repo>/<path>` и строки `uploaded to <repo>/<path>`.
Также он получает непросроченные Forgejo Actions artifacts через API.
...
Сейчас это каркас без реального playbook oVirt. Для production-сценариев он пока
не годится.
...
- Workflow объявляет все рабочие inputs, переданные из `with`.
...
Если редактор начнёт использовать этот файл как встроенную справку, показывайте
ограничения рядом с соответствующими полями.
